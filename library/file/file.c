 ...
 */
static int treedisk_setsize(inode_store_t* this_bs, unsigned int ino,
                            block_no nblocks) {
  return -1;
}
 ...
 */
static int treedisk_read(inode_store_t* this_bs, unsigned int ino,
  ...
  for (;;) {
    ...
    unsigned int index =
        log_shift_r(offset, nlevels * log_rpb) % REFS_PER_BLOCK;
    b = tib->refs[index];
  }
  return 0;
}
 ...
 */
static int treedisk_write(inode_store_t* this_bs, unsigned int ino,
                          block_no offset, block_t* block) {
  struct treedisk_state* ts = this_bs->state;
  int dirty_inode = 0;
  ...
  if (snapshot->inode->nblocks == 0) {
    ...
    while (nlevels_after > nlevels) {
      ...
      dirty_inode = 1;
      if ((*ts->below->write)(ts->below, ts->below_ino, indir, (block_t*)&tib) <
          0) {
  ...
  /* If the inode block was updated, write it back now.
     */
  if (dirty_inode)
    if ((*ts->below->write)(ts->below, ts->below_ino, snapshot->inode_blockno,
                            (block_t*)&snapshot->inodeblock) < 0) {
 ...
 * block on it.
 */
block_no setup_freelist(inode_store_t* below, unsigned int below_ino,
 ...
 */
int treedisk_create(inode_store_t* below, unsigned int below_ino,
