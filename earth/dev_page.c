...
static int cache_eviction() {
  ...
  earth->disk_write(frame_id * NBLOCKS_PER_PAGE, NBLOCKS_PER_PAGE,
                    pages_start + PAGE_SIZE * idx);
  return idx;
}
...
char* paging_read(int frame_id, int alloc_only) {
  ...
  int free_idx = -1;
  for (int i = 0; i < ARTY_CACHED_NFRAMES; i++) {
    if (cache_slots[i] == -1 && free_idx == -1)
      free_idx = i;
    if (cache_slots[i] == frame_id)
      return pages_start + PAGE_SIZE * i;
  }
  ...
  if (!alloc_only)
    earth->disk_read(frame_id * NBLOCKS_PER_PAGE, NBLOCKS_PER_PAGE,
                     pages_start + PAGE_SIZE * free_idx);
