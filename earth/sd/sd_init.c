...
enum { SD_TYPE_SD1, SD_TYPE_SD2, SD_TYPE_SDHC, SD_TYPE_UNKNOWN };
...
static void sd_check_capacity() {
  ...
  if (sd_exec_cmd(cmd58))
    FATAL("SD card cmd58 fails");
  for (int i = 0; i < 4; i++)
    payload[3 - i] = recv_data_byte();

  if ((payload[3] & 0xC0) == 0xC0)
...
void sdinit() {
  ...
  if (0 != sd_check_type())
    FATAL("Fail to check SD card type");

  char acmd41[] = {0x69, (SD_CARD_TYPE == SD_TYPE_SD2) ? 0x40 : 0x00,
                   0x00, 0x00,
                   0x00, 0xFF};
  ...
  if (SD_CARD_TYPE != SD_TYPE_SDHC)
    FATAL("Only SDHC/SDXC supported");
}
